 ...
 * @access public
 */
class Cosmobit_Customizer_Range_Control extends WP_Customize_Control {
	...
	public function enqueue() {
		...
		wp_enqueue_script(
			'cosmobit-controls',
			COSMOBIT_THEME_INC_URI . '/customizer/controls/js/controls.js',
			array(
				'jquery',
				'customize-base',
				'jquery-ui-button',
		...
		wp_enqueue_style( 'cosmobit-controls', COSMOBIT_THEME_INC_URI . '/customizer/controls/css/controls.css' );
		wp_enqueue_style( 'font-awesome', COSMOBIT_THEME_URI . '/assets/vendors/css/font-awesome.min.css' );
	}
	
	/**
	 * Cosmobit_Customizer_Range_Control constructor.
	 *
 ...
 */
function cosmobit_sanitize_range_value( $input ) {
	if ( cosmobit_is_json( $input ) ) {
 ...
 * @param  string $string Input.
 */
function cosmobit_is_json( $string ) {
	return is_string( $string ) && is_array( json_decode( $string, true ) ) ? true : false;
}
